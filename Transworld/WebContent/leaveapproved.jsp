...
Statement st,st1,st2,st3,stInsrt;
String sql=null;
String sqlHistory="";
Boolean flag=false;
...
ResultSet rs1=st1.executeQuery(sql2);
	...
	if(type=="update"||type.equalsIgnoreCase("update"))
	{
		...
		for(int a1=0; a1<appliedDays.size(); a1++)
		{
			...
			HOD="-";
			String sqlHod="select * from db_leaveapplication.t_leaverequest where emailid='"+empEmail.get(a1)+"' and firstdayofleave='"+frstDay.get(a1)+"' and daysapplied='"+appliedDays.get(a1)+"' ";
			...
			sqlHistory="insert into t_leaveApplicationHostory (FirstDayOfLeave, leaveType, empName, empMail, dept, HOD, changedBy, updatedStatus) values ('"+frstDay.get(a1).toString()+"','"+typeLeave.get(a1)+"','"+nameEmp.get(a1)+"','"+emailid+"','"+dept.get(a1)+"','"+HOD+"','"+session.getAttribute("empname").toString()+"','Accepted')";
	...
	else if(type=="not"||type.equalsIgnoreCase("not"))
	{
		...
		for(int a1=0; a1<appliedDays.size(); a1++)
		{
			...
			sqlHistory="insert into t_leaveApplicationHostory (FirstDayOfLeave, leaveType, empName, empMail, dept, HOD, changedBy, updatedStatus) values ('"+frstDay.get(a1).toString()+"','"+typeLeave.get(a1)+"','"+nameEmp.get(a1)+"','"+emailid+"','"+dept.get(a1)+"','"+HOD+"','"+session.getAttribute("empname").toString()+"','NotAccepted')";
			System.out.println("sql--->"+sqlHistory);
			int p=stInsrt.executeUpdate(sqlHistory);
		}
	...
	else if(type=="revert"||type.equalsIgnoreCase("revert"))
	{
		...
		for(int a1=0; a1<appliedDays.size(); a1++)
		{
			...
			sqlHistory="insert into t_leaveApplicationHostory (FirstDayOfLeave, leaveType, empName, empMail, dept, HOD, changedBy, updatedStatus) values ('"+frstDay.get(a1).toString()+"','"+typeLeave.get(a1)+"','"+nameEmp.get(a1)+"','"+emailid+"','"+dept.get(a1)+"','"+HOD+"','"+session.getAttribute("empname").toString()+"','Pending')";
			System.out.println("sql--->"+sqlHistory);
			int p=stInsrt.executeUpdate(sqlHistory);
		}
		innerflag=false;
	...
	else
	{
		...
		for(int a1=0; a1<appliedDays.size(); a1++)
		{
			...
			sqlHistory="insert into t_leaveApplicationHostory (FirstDayOfLeave, leaveType, empName, empMail, dept, HOD, changedBy, updatedStatus) values ('"+frstDay.get(a1).toString()+"','"+typeLeave.get(a1)+"','"+nameEmp.get(a1)+"','"+emailid+"','"+dept.get(a1)+"','"+HOD+"','"+session.getAttribute("empname").toString()+"','Cancel')";
			System.out.println("sql--->"+sqlHistory);
			int p=stInsrt.executeUpdate(sqlHistory);
		}
...
if(i>1)
{
 ...
 pass);
		               ...
		               recipientAddrs1[0] = new InternetAddress(session.getAttribute("email").toString());
		               
		               //----HR mail-------------------------------------------------
		               String hr="";
		               String sqlHR="select * from db_leaveapplication.t_leaveadmin where URole='HR' and Status='Yes'";
