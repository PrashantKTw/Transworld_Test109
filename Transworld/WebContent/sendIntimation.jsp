...
System.out.println("========"+inwrdRef+"====="+dept+"=============="+toName+"====="+senderCmts+"====="+user1);
...
//============================================================
	try
	{
		...
		for (String t : t2)
		{
			//sqlInsrt="insert into db_leaveapplication.t_inwardfollowup(iwrefno,docReceiver, docSender,senderRemarks,actionondoc) values('"+inwrdRef+"','"+t+"','"+session.getAttribute("empname").toString()+"','"+remark+"','forward')";
		...
		/*
		if(cat.equals("Bills") || cat=="Bills")
		{
			sqlInsrt="INSERT INTO db_leaveapplication.t_inwarddetails (InwardNo ,InwardRefNo ,InwardDate ,InwardTime ,FromParty ,PartyRefNo ,ToEmploy ,ToEmailID ,Subject ,Dept ,Category,BillNo,BillAmt,BillDate,BillDueDate,FileName ,FileLocation,Enteredby,EnteredById,EmailSend,ActionDate,senderRemark,InwardStatus)VALUES('"+iwno+"', '"+refno+"', '"+iwDate+"' , '"+iwTime+"' , '"+fParty+"', '"+pRef+"', '"+toEmp+"', '"+toEmailid+"', '"+sub+"', '"+dept1+"', '"+cat+"','"+blno+"','"+blamt+"','"+bldate+"','"+blduedate+"'  ,'"+fname+"','"+floc+"', '"+entBy+"','"+session.getAttribute("empID").toString()+"' ,'"+mailSend+"','"+aDate+"','"+remark+"','inwarded')";	
		}
		else
		{
			sqlInsrt="INSERT INTO db_leaveapplication.t_inwarddetails (InwardNo ,InwardRefNo ,InwardDate ,InwardTime ,FromParty ,PartyRefNo ,ToEmploy ,ToEmailID ,Subject ,Dept ,Category,FileName ,FileLocation,Enteredby,EnteredById,EmailSend,ActionDate,senderRemark,InwardStatus)VALUES('"+iwno+"', '"+refno+"', '"+iwDate+"' , '"+iwTime+"' , '"+fParty+"', '"+pRef+"', '"+toEmp+"', '"+toEmailid+"', '"+sub+"', '"+dept1+"', '"+cat+"','"+fname+"','"+floc+"', '"+entBy+"','"+session.getAttribute("empID").toString()+"' ,'"+mailSend+"','"+aDate+"','"+remark+"','inwarded')";
		}
		*/
	}
	catch(Exception e)
...
//===Send Mail================================================
	...
	try
	{
	         ...
	         //      DataSource source = new FileDataSource("");//give file path on ftp
	...
	//String empname=session.getAttribute("uname").toString();
	//out.println("====="+empname+"======="+session.getAttribute("uname").toString()+"============"+senderMail+"======="+redirectMsg+"====");
