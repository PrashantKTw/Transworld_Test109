		...
		<tr>
			...
			<form action="" name="form1" id="form1" method="get">
				<td align="left"><font size="2" color="#16B836"><a
					...
					size="2" color="#F6E712"><a href="leaveLink.jsp?st=cancel1"
						style="color: #F6E712" onclick="getTypeCancld();">Canceled
							leave</a></font></td>
				<%
		...
		<tr>
			<td align="center"><select name="status">
				...
				value="<%=today%>" readonly /> <script type="text/javascript">
			        ...
			        </script> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp <input type="text"
				id="toDate" name="toDate" size="13" value="<%=today%>" readonly /> <script
			...
			%>
							...
							<td align="center"><a href="#"
								...
								onclick="return onSubmitForNot('<%=rs.getString("srno") %>','<%=i%>')"><font
									size="2" color="#09195B">Reject</font></a><br> <a href="#"
...
%>
			...
			else if(data.equals("Pending1") || data.equals("Approved1") || data.equals("NotApproved1") || data.equals("cancel1"))
			{
				...
				//out.println(datetoday);
				String sql="";
			//	sql="select * from t_leaverequest where  TypeValue='"+typevalue+"' and emailid='"+username+"' and status='"+data+"' ";
			
				int len=data.length();
				data=data.substring(0,len-1);
				
				sql="select * from t_leaverequest where  TypeValue='"+typevalue+"' and status='"+data+"' order by firstdayofleave desc";
				ResultSet rs=st.executeQuery(sql);
				System.out.println("Qry============>>"+sql);
				int i=1;
				while(rs.next()){
				...
				while(rs.next()){
					...
					%>
						...
						if((rs.getString("status").equalsIgnoreCase("Approved")&& uroleofuser.equalsIgnoreCase("emp"))
							...
							<a href="#"
								onclick="return onSubmitForNot('<%=rs.getString("srno") %>','<%=i%>')"><font
									size="2" color="#09195B">Reject</font></a> <br>
							<a href="#"
								onclick="return onSubmitForCancle('<%=rs.getString("srno") %>','<%=i%>')"><font
									size="2" color="#09195B">Cancel</font></a></td>
							<%} %>
